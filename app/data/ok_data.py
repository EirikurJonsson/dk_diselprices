...
def ok_data():
    ...
    """
    enddate = datetime.now().strftime('%Y-%m-%dT%H:%m:00.000Z')
    startdate = (datetime.now() - timedelta(days = 360)).strftime('%Y-%m-%dT%H:%m:00.000Z')


    products = {'benzin95': 536, 'diesel': 231}
    url =f"https://www.ok.dk/privat/produkter/ok-kort/prisudvikling/download?moms=true&varenr=536&from={startdate}&to={enddate}&inklAfgifter=true&per1000liter=false&pumpepris=false&perKg=false"
